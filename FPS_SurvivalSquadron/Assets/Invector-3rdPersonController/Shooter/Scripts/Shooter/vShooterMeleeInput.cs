...
namespace Invector.vCharacterController
{
    ...
    public class vShooterMeleeInput : vMeleeCombatInput, vIShooterIKController, PlayerController.vILockCamera
    {
        ...
        { 
            get 
            { 
                if (!shooterManager.hipfireShot && _aimTimming > 0 || (isReloading&& !shooterManager.keepAimingWhenReload) || isEquipping || lockShooterInput)
                {
                    _aimTimming = 0; 
                    return false; }  
                return shooterManager.hipfireShot && ((_aimTimming > 0||(shotInput.GetButton() && shooterManager.CurrentWeapon != null)) || (!isAimingByInput && shootCountA > 0)); 
            } 
        }
        ...
        protected virtual void AlignArmToAimPosition(bool isUsingLeftHand = false)
        {
            ...
            if (CurrentActiveWeapon )
            {
                if (!shooterManager.isShooting)
                {
                    ...
                    arm.UpdatePose(CurrentActiveWeapon.aimReference);
                }
                else
                {
                    arm.RestoreLastPose(CurrentActiveWeapon.aimReference);
                }
                ...
                arm.AlighPoseToAimPoint(targetArmAlignmentPosition,armAlignmentWeight, CurrentActiveWeapon.alignRightUpperArmToAim, CurrentActiveWeapon.alignRightHandToAim);
        ...
        Vector3 scopeLocalPosition;
        Vector3 scopeLocalForward;
        ...
        protected virtual void UpdateAimHud()
        {
            ...
            if (controlAimCanvas.scopeBackgroundCamera && CurrentActiveWeapon.scopeTarget)
            {
                if (isUsingScopeView)
                {
                    ...
                    if(!shooterManager.isShooting)
                    {
                       scopeLocalForward =  arm._aimReference.InverseTransformDirection(CurrentActiveWeapon.scopeTarget.forward);
                       scopeLocalPosition = arm._aimReference.InverseTransformPoint(CurrentActiveWeapon.scopeTarget.position);                      
                    }
                    ...
                    var scopeLookDirection = Vector3.Lerp(controlAimCanvas.scopeBackgroundCamera.transform.forward, arm._aimReference.TransformDirection(scopeLocalForward),scopeDirectionWeight = Mathf.Lerp(scopeDirectionWeight, 1.001f, (10f / CurrentActiveWeapon.shootFrequency) * Time.deltaTime));
                    var scopePosition = Vector3.Lerp(controlAimCanvas.scopeBackgroundCamera.transform.position, arm._aimReference.TransformPoint(scopeLocalPosition), weight);
